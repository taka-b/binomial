...
import traceback
import utility_functions_02 as uf

mpl.rcParams['agg.path.chunksize'] = 5000000000
...
def json_init(filename):
    ...
    global config, CSV_OUTPUT, FIG_OUTPUT, RestartFile
    ...
    Fig_from_CSV = config["settings"]["FiguersFromCSV_interval"]
    ...
    Optimization = config["settings"]["Optimization"]
...
class Plot:
    ...
    def resultsPlot_02(self, timeM, step, unit, fName, printNum, ShowFigure, utils):
        ...
        ax.set_xlabel(f"step ({unit})", fontsize=14)
        ax.set_ylabel("Number", fontsize=14)
        ax.set_yscale(self.plotScale)
        ax.legend(loc='best')
        ax.set_title('Number of the element', loc='center', fontsize=14)
    ...
    def resultsPlot_04(self, tRange, step, unit, fName, printNum, ShowFigure, utils, index): # 2024.4.28
        ...
        # ISO 8601 形式でフォーマット（例: '2023-04-12T14:30:00.000123'）
        # iso_format_time = current_time.strftime('%Y-%m-%dT%H_%M_%S')
