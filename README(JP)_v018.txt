...
2. フォルダの準備
   ...
   メインホルダーの中に別の入力ファイル用のホルダーを作成して、入力ファイルを格納します。(参照：Readme_v018_Figuers.pdf中のFigure１)
...
3.5 binomial_parameter_07.json
   ...
   CSVファイル出力間隔で値が出力され、 "NO"ではファイルが作られません。
        ...
        "making_folder":    ["YES" or "NO"], 結果出力用のフォルダを作らない場合は "NO"を選択してください。
　　　　　　　　　　　　　　　　　　　　　　その場合は、以下の結果出力の"YES"にかかわらず結果ファイルは作られません。
...
　Figure2.は、定義領域の概念図です。一番外側には、今までの定義ファイルで定義される領域があり、
その中に*Set_01で定義される領域が存在し、さらにその内側に*Set_02で領域が定義されます。
*Set_01や*Set_02は、Figure3.のように複数個定義することも可能となっています。
...
Figure4.に例を示します。大域領域のファイルをSet_401.txtとしています。
下記では、大域領域に１つの*Set_01領域があり、さらにその中に１つの*Set_02領域がある例になっています。
...
Figure4.には、それぞれの領域内の要素を図示しました。この図を見ると各領域間に要素のやり取りが無いように見えますが、
以下にみるように、膜タンパクが定義されて物質のやり取りが可能になっています。また、拡散により移動も定義できます。
これらの物質移動の定義は、5.3.2や5.3.3で詳しく説明されています。
Figure4.のaとa:=は物質としては同じものですが、領域の移動により名前の変更が行われていることに注意してください。
Figure5.は、下記ファイルにおける各要素の移動反応の様子を模式的に書いたものです。
...
5.3.5. 複数の領域が存在する場合 (Set_402.txt)
   ...
   以下に示すSet_402.txtは、*Set_01内に2種類の領域がそれぞれ2つと3つある場合で、かつ*Set_02内では、
   それぞれの*Set_01内の領域に、違った*Set_02をそれぞれ2つと1つずつ持つ定義の例です。
   この入力ファイルが読まれると、大域領域内には5つの*Set_01領域が作られ、C1内にはそれぞれ2個ずつのM1が作られ、
   C2内にはそれぞれそれぞれ1個のM2が作られることになります。Figure6.のその様子を示しました。大域領域と合わせると合計
   13の領域があることになります。
...
　　　　1.Spyder
           ...
           実行方法の1つは、anaconda3でspyderを使用します。 Spyderからメインのプログラムファイルbinomial_v018.pyを開きます。
...
また、CSVファイルも保存されます。CSVファイルには4種類あります：
   ...
   inp_immune_323_probability.csv：ランダムに整数を返すrandom.binomial(n, p)に与えられる変数、nとpを出力します。
